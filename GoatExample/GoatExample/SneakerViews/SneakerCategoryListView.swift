...
struct SneakerCategoryListView: View {
    
    @Binding var showAll: Bool
    @Binding var sneakerBrand: String
    
    let screenSize: CGSize
    let resultCategories: [SneakerCategory]
        ...
        return VStack {
            List {
                ForEach(self.resultCategories, id: \.id) { resultCategory in
                    SneakerCategoryCellView(scrennSize: screenSize, showAll: $showAll, sneakerBrand: $sneakerBrand, resultCategoryVM: resultCategory)
                }
            }
